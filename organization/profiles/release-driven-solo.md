...
tastile-openapi の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues / Pull Requests: durable contract-maintenance work and review evidence
- release goal / target version is carried by the active release branch and linked GitHub work
- do not mirror tastile-core implementation facts into this repository as a second canonical implementation source
...
- `tastile-core/crates-v1/api/src/openapi.rs` remains the generator source authority.
- `openapi.yaml` is generated contract output and must not be hand-edited.
- a valid contract update binds: core source SHA -> generated OpenAPI content -> openapi repository SHA/tag -> tastile-root submodule pointer -> affected consumer regeneration evidence.
- consumer generated types are downstream evidence, not canonical contract source.
- current active release branch is `release-1.0.1`; ticket PRs target it until the release changes.
- main integration remains release-branch-only and merge-commit-only.

## Agent Skills lifecycle

- current `rebuildup/project-init` is managed with `bunx skills`.
- canonical layout: `.agents/skills/` + `.claude/skills/`.
- fresh bootstrap: `mise run skills-bootstrap`.
