# tastile-openapi Constitution

この文書は、tastile-openapi の開発組織における最上位 contract です。
...
tastile-openapi の開発組織の目的は current procedure への服従ではありません。
...
This repository is the canonical distributed OpenAPI contract for Tastile consumers, but the Rust/utoipa source of truth for generation lives in `tastile-core`. Generated contract identity, source-core identity, submodule pointer identity, and consumer-generated-code identity must remain explicit.

## Upstream source

Governance was reconciled from `rebuildup/project-init@release-0-3-0` (`57fb4a2e5abe6f52e4fd9cb2cb88234496e47d4b`). Agent Skills track current project-init through the project-local Skills CLI.
